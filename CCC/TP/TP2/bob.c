...
int main(){
    // création d'une paire de clés et sauvegarde de la clé publique
    creation_paire_cles("cle_publique.txt", "cle_privee.txt");
    // lecture de la clé publique
    int e, n;
    lecture_cle_pub("cle_publique.txt", &e, &n);
    // chiffrement d'un fichier avec la clé publique
    chiffrement_texte(e, n, "fichier.txt", "fichier_chiffre.txt");
    // déchiffrement du fichier chiffré avec la clé privée
    int p = 1061;
    int q = 1069;
    int d = 54457;
    dechiffrement_texte(p, q, d, "fichier_chiffre.txt");
    return 0;
}